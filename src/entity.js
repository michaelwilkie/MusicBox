class Entity
{
    ...
    {        
        ...
        if (checkCameraCollision(camera, this))
        {
            ctx.drawImage(this.img,
                ...
                0, 
                this.w, this.h, 
                this.pos.x - camera.x + camera.screenpartitionx, 
    ...
    update()
    {
        if (this.pos.x + this.w > level.width)  { this.xCollide = true; this.vel.x *= -1; this.pos.x = level.width  - this.w;}
        else if (this.pos.x < 0)                { this.xCollide = true; this.vel.x *= -1; this.pos.x = 0;                    }
        else this.xCollide = false;
        if (this.pos.y + this.h > level.height) { this.yCollide = true; this.vel.y *= -1; this.pos.y = level.height - this.h;}
